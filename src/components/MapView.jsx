...
const MapView = ({ selectedSpot, spots = [] }) => {
    ...
    }, [ready, spots, isMapLoaded])

    useEffect(() => {
        // 1. 지도/SDK 준비 안됐거나, 선택된 장소가 없으면 중단
        if (!ready || !selectedSpot || !mapInstanceRef.current || !window.kakao?.maps || !isMapLoaded) return

        const map = mapInstanceRef.current

        // 2. 선택된 장소의 좌표로 지도를 이동
        ...
        // 3. 해당 장소의 마커를 찾아서 정보창을 엶
        const marker = markersRef.current.find(
            (m) => m.spot?.lat === selectedSpot.lat && m.spot?.lng === selectedSpot.lng
        )
        ...
        if (marker && infoRef.current) {
            infoRef.current.setContent(`
                <div class="p-2 min-w-[160px]">
                    ...
                    <div class="text-xs text-slate-500 mt-1">${selectedSpot.addr || ''} ${selectedSpot.detail || ''}</div>
    ...
    }, [selectedSpot, ready, isMapLoaded])
